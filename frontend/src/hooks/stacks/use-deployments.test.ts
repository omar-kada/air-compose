...
describe('getDeploymentsQueryOptions', () => {
  ...
  it('queryFn calls deployementAPIList with pageParam', async () => {
    const options = getDeploymentsQueryOptions();
    const result = await (options.queryFn as unknown as AnyFunction)({
  ...
  it('select flattens pages into Deployment[]', () => {
    ...
    const data = {
      pageParams: [],
      pages: [{ data: { items: [{ id: 1 }, { id: 2 }] } }, { data: { items: [{ id: 3 }] } }],
    };
    expect((options.select as unknown as AnyFunction)(data)).toEqual([
      { id: 1 },
      { id: 2 },
      { id: 3 },
    ]);
  ...
  it('getNextPageParam returns undefined when endCursor is empty', () => {
    ...
    expect((options.getNextPageParam as unknown as AnyFunction)(lastPage)).toBeUndefined();
  });

  it('getNextPageParam returns offset object when endCursor is present', () => {
    const options = getDeploymentsQueryOptions();
    const lastPage = { data: { items: [], pageInfo: { endCursor: 'xyz' } } };
    expect((options.getNextPageParam as unknown as AnyFunction)(lastPage)).toEqual({
      limit: 10,
      offset: 'xyz',
    });
  });
