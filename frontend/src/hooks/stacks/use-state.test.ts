...
describe('getStateQueryOptions', () => {
  ...
  it('select extracts data.data', () => {
    getStateQueryOptions();
    const select = (mockGetOpts as unknown as Mock).mock.calls[0][0].query.select;
  ...
  it('refetchInterval returns diffMs for future nextDeploy', () => {
    getStateQueryOptions();
    const refetchInterval = (mockGetOpts as unknown as Mock).mock.calls[0][0].query.refetchInterval;
