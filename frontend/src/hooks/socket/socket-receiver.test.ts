...
describe('createSocketReceiver', () => {
  it('routes log messages to onLogEvent', () => {
    ...
    const onEvent = createSocketReceiver(client as any);
    const msg = { data: JSON.stringify({ kind: ServerMessageLogKind.log, value: 'test' }) };
    onEvent(msg as any);
  ...
  it('routes previousLogs to onLogEvent', () => {
    const client = mockClient();
    const onEvent = createSocketReceiver(client as any);
    const msg = { data: JSON.stringify({ kind: ServerMessagePreviousLogsKind.previousLogs, value: [] }) };
    onEvent(msg as any);
    const parsed = JSON.parse(msg.data);
    expect(mockOnLogEvent).toHaveBeenCalledWith(parsed, client);
  });
  ...
  it('invalidates queries on newDeployment', () => {
    const client = mockClient();
    const onEvent = createSocketReceiver(client as any);
    const msg = { data: JSON.stringify({ kind: ServerMessageNewDeploymentKind.newDeployment }) };
    onEvent(msg as any);
  ...
  it('calls onLogEvent for event messages with deploymentId', () => {
    const client = mockClient();
    const onEvent = createSocketReceiver(client as any);
    const msg = { data: JSON.stringify({ kind: ServerMessageEventKind.event, value: { deploymentId: '123', type: EventType.DEPLOYMENT_STARTED } }) };
    onEvent(msg as any);
    expect(client.invalidateQueries).toHaveBeenCalled();
  });
  ...
  it('calls toast.error on ERROR event', () => {
    const client = mockClient();
    const onEvent = createSocketReceiver(client as any);
    const msg = { data: JSON.stringify({ kind: ServerMessageEventKind.event, value: { type: EventType.ERROR, msg: 'fail' } }) };
    onEvent(msg as any);
  ...
  it('throws on unhandled message kind', () => {
    ...
    expect(() => onEvent(msg as any)).toThrow('Unhandled');
  });
});
