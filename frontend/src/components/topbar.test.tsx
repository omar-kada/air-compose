...
vi.mock('@/hooks', () => ({
  ...
  getNotificationsQueryOptions: () => ({ queryKey: ['notifications'], queryFn: () => ({ pages: [] }) }),
  getFeaturesQueryOptions: () => ({ queryKey: ['features'], queryFn: () => ({}) }),
  getSettingsQueryOptions: () => ({ queryKey: ['settings'], queryFn: () => ({}) }),
...
describe('Topbar', () => {
  ...
  it('T3: shows logged in user', async () => {
    ...
    expect(screen.getByText((content) => content.includes('testuser'))).toBeTruthy();
  });
  ...
  it('T6: toggles theme when clicking dark mode', async () => {
    renderTopbar();
    await openUserDropDown();
    await userEvent.click(findMenuItem('dark mode')!);
  ...
  it('T8: signs out when clicking sign out', async () => {
    renderTopbar();
    await openUserDropDown();
    await userEvent.click(findMenuItem('sign out')!);
