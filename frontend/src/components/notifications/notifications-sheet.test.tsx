...
describe("NotificationSheet", () => {
  ...
  it("renders sheet title and description when opened", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("translated:NOTIFICATIONS.NOTIFICATIONS"),
      ).toBeTruthy();
    });
    expect(
      screen.getByText("translated:NOTIFICATIONS.DESCRIPTION"),
    ).toBeTruthy();
  });
  ...
  it("navigates to deployment when notification item clicked", async () => {
    ...
    const targetItem = Array.from(items).find((i) =>
      i.textContent?.toLowerCase().includes("dep1"),
    );
