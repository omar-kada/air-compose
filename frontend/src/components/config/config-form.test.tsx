...
describe("ConfigForm", () => {
  it("renders service cards for each service", () => {
    ...
    expect(
      container.querySelector('[data-slot="service-card"][data-name="web"]'),
    ).not.toBeNull();
  });
  ...
  it("renders global env vars form", () => {
    const { container } = render(<TestWrapper />);
    expect(
      container.querySelector('[data-slot="env-vars-array-form"]'),
    ).not.toBeNull();
  });
  ...
  it("renders add service button when not disabled", () => {
    render(<TestWrapper />);
    expect(
      screen.getByText("translated:CONFIGURATION.FORM.ADD_SERVICE"),
    ).toBeTruthy();
  });
  ...
  it("hides add service button when disabled", () => {
    ...
    expect(
      screen.queryByText("translated:CONFIGURATION.FORM.ADD_SERVICE"),
    ).toBeNull();
  });
  ...
  it("renders multiple service cards", () => {
    ...
    expect(
      container.querySelectorAll('[data-slot="service-card"]'),
    ).toHaveLength(2);
  });
  ...
  it("adds a new service when Add Service button is clicked", () => {
    ...
    fireEvent.click(
      screen
        .getByText("translated:CONFIGURATION.FORM.ADD_SERVICE")
  ...
  it("removes a service when Remove button is clicked", () => {
    const { container } = render(<TestWrapper />);
    fireEvent.click(
      container.querySelector('[data-slot="remove-service"]') as HTMLElement,
    );
    expect(
      container.querySelectorAll('[data-slot="service-card"]'),
    ).toHaveLength(0);
