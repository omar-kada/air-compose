...
func (sess *session) dispatch(ctx context.Context, messageType string, raw json.RawMessage) {
	switch messageType {
	case string(api.ClientMessageStartLogsKindStartLogs):
		var msg api.StartLogsMessage
	...
	case string(api.ClientMessageEndLogsKindEndLogs):
		sess.cancelLogs()
...
func (sess *session) handleStartLog(ctx context.Context, msg api.StartLogsMessage) {
	...
	go func() {
		...
		for lines := range subscribeToLogs(logCtx, int(msg.PreviousLines)) {
			if len(lines) > 1 {
				...
				if err := wsjson.Write(logCtx, sess.conn, api.ServerMessagePreviousLogs{
				...
				if err := wsjson.Write(logCtx, sess.conn, api.ServerMessageLog{
