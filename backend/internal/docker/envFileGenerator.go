...
// OverrideHeader is the comment used to mark the start of the section
// in the .env file that is managed by AirCompose.
const OverrideHeader = "# The next values are generated by AirCompose :"
	...
	if envMap != nil {
		for key, rawLine := range envMap.AllFromFront() {
			if serviceCfg.Has(key) {
				fmt.Fprintf(&content, "# %s # Overridden by AirCompose \n", rawLine)
			} else {
				fmt.Fprintf(&content, "%s\n", rawLine)
			}
		}
	}
	fmt.Fprintln(&content, OverrideHeader)
...
func parseEnvFile(path string) (*orderedmap.OrderedMap[string, string], error) {
	...
	for index, rawLine := range lines {
		...
		if strings.Contains(line, OverrideHeader) {
			break
		}
